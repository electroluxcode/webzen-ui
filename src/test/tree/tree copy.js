import "https://cdn.jsdelivr.net/npm/webzen-ui/wz-icon/";
...
class tree {
    ...
    asyncData(parent, data) {
        ...
        const $arrow = document.createElement("i");
        $arrow.setAttribute("class", `node-arrow fa fa-angle-right open`);
        ...
        if (!this.TreePrefData[parent].children) {
            ...
            dom?.querySelector(`.node-arrow`)?.classList.add("fa");
            dom?.querySelector(`.node-arrow`)?.classList.add(" fa-arrow-right");
            setTimeout(() => {
