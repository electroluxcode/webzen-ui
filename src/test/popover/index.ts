...
declare global {
	interface HTMLElement {
	  popover: any; // 你可以将 "any" 替换为属性的实际类型
	}
  }
export default class PopOver extends Base {
	...
	#pop;
	#isHover:boolean;
	#isHoverPoper:boolean;
	#timer:any;
	target:any
	_documentClickEvent:any
	...
	#disconnect(target:any) {
		// wz包裹的元素不用监听
		if (target.parentNode?.tagName?.startsWith("wz-")) return;
	...
	// 初始化
	init(this:any,target:any, option:any) {
		if (!target) return;
	...
	get #targetAll() {
		...
		if (target) {
			return [...(this.getRootNode() as any).querySelectorAll(target)]
		} else {
			return [this.previousElementSibling || this.parentNode]
		}
	}

	#bind(this:any,target:any) {
		if (!target.clientWidth) return;
		this.init(target, {
			dir: this.dir,
		...
		if (this.trigger.includes('contextmenu')) {
			target.addEventListener('contextmenu', (ev:any) => {
				ev.preventDefault();
				if (this.disabled) return;
	...
	render(this:any){
		...
		this.targetList.forEach((target:any) => {
			this.#bind(target)
		});
	}
	...
	disconnectedCallback(this:any) {
		if (this._documentClickEvent.length && !this.isConnected) {
			this._documentClickEvent.forEach((event:any) => {
