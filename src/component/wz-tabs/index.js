...
function dataJsSwitch(mode, that) {
    ...
    let first_index = that.querySelector('[slot="tabs_index"]');
    if (!first_index) {
        console.log("找不到元素");
        return;
    }
  ...
  `;
    if (mode == "top_card") {
        that.querySelectorAll('[slot="tabs_index"]').forEach((e) => {
            e.style = `
      font-size: 12px;
    ...
    if (mode == "top" || mode == "top_middle") {
        ...
        divline.style.top = that.shadowRootInit.querySelector(".tab").offsetHeight - 2 + "px";
    }
    if (mode == "left") {
    ...
    let first_value = that.querySelector(`[tab_value_key="${first_index.getAttribute("tab_item_key")}"]`);
    // first_value.classList.add("move_left")
    first_value.style.display = "block";
    // 点击 事件
    that.shadowRootInit.querySelector(".tab").addEventListener("click", (e) => {
        if (JSON.stringify(e.target.getAttribute("tab_item_key"))) {
            // tab_index 的切换
            Array.from(tab_indexs).forEach((e) => {
      ...
      `;
        ...
        `;
            ...
            let target_value = that.querySelector(`[tab_value_key="${target_index}"]`);
...
function dataCssSwitch(tabs_params, that) {
    if (!tabs_params) {
        console.log("参数传递有漏");
        return;
    }
